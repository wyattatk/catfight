...
void R_AddIQMSurfaces( trRefEntity_t *ent ) {
	...
	if ( (ent->e.frame >= data->num_frames)
	     ...
	     || (ent->e.oldframe >= data->num_frames)
	     || (ent->e.oldframe < 0) ) {
		// catfight: silent when num_frames is 0, because a static IQM asking
		// for frame 0 is not a mistake. See renderergl1/tr_model_iqm.c for the
		// full reasoning and the numbers. Changed here too even though catfight
		// ships opengl1 -- a divergence that exists in one renderer and not the
		// other is a trap for whoever switches.
