...
Raise EV_FOOTSTEP at a cadence set by how fast we are actually moving.

IN PMOVE, NOT IN CGAME, and that is the whole reason it is here rather than
somewhere easier. A footstep is information other people act on -- it is how
you know somebody is coming -- so it has to happen at the same instant for the
person taking the step and for everybody listening. Raised here it is predicted
locally, replicated through the entity state, and identical on the server; put
in cgame it would be a local guess that drifted from what the shooter heard.

The accumulator lives in ps->bobCycle, whose comment in the engine's own header
says "for view bobbing and footstep generation" -- it is replicated and delta
compressed for free. It wraps at 256 and a step fires on each half, so two per
cycle, one per foot.

Distance rather than time: bobmove scales with actual horizontal speed, so
steps slow down as you do instead of the cadence being right at exactly one
velocity.
============
*/
static void PM_Footsteps( void ) {
	...
	// Airborne, dead or spectating: the jump and landing events cover those,
	// and PM_CrashLand is what makes an arrival audible.
	if ( !pml.walking || pm->ps->pm_type != PM_NORMAL ) {
		return;
	}

	// Deliberately quiet. See CF_STEP_BOB_RUN in cf_shared.h -- this is the
	// design decision, and it is these two lines.
	if ( ( pm->ps->pm_flags & PMF_DUCKED ) || ( pm->cmd.buttons & BUTTON_WALKING ) ) {
		return;
	}

	/*
	Speed is measured from VELOCITY, not from the command. Sliding to a halt,
	being pushed, or walking into a wall all leave forwardmove set while you
	are not actually covering ground -- keying off the button would have you
	tapping your feet against the wall you are stuck on.
	*/
	xyspeed = sqrt( pm->ps->velocity[0] * pm->ps->velocity[0]
	                + pm->ps->velocity[1] * pm->ps->velocity[1] );
	if ( xyspeed < CF_STEP_MIN_SPEED ) {
		return;
	}

	bobmove = CF_STEP_BOB_RUN * ( xyspeed / CF_RUN_SPEED );

	old = pm->ps->bobCycle;
	pm->ps->bobCycle = (int)( old + bobmove * pml.msec ) & 255;

	// Crossing either half of the cycle is a foot landing. The +64 rotates the
	// test off the wrap point so that one step is not split across it.
...
static void PmoveSingle( pmove_t *pmove ) {
	...
	/*
	A spectator flies, and flies through things.

	PM_SPECTATOR used to fall through to the walking code below, which made an
	eliminated player a floating, colliding body: they drifted along the floor,
	bumped into walls, and could not get above anything. That was survivable
	while being dead meant only watching.

	It is not survivable now that a dead player can still command his companion.
	He keeps playing after he is killed -- and the reason that is fair is that
	his surviving teammate is a bot, so the information he gathers as a camera is
	what gives the side a chance at all. A camera that cannot leave the room it
	died in gathers nothing.

	PM_NoclipMove is exactly the right movement and already exists; the only
	difference from PM_NOCLIP is who is allowed to be in this state.
	*/
	if ( pm->ps->pm_type == PM_NOCLIP || pm->ps->pm_type == PM_SPECTATOR ) {
		PM_NoclipMove();
		pm->ps->groundEntityNum = ENTITYNUM_NONE;

		// PM_CheckDuck is skipped on this path, so the eye would otherwise keep
		// whatever height it had when the body died -- including the crouched
		// one, which puts a free camera oddly low for no reason anybody could
		// see.
		pm->ps->viewheight = CF_VIEWHEIGHT_STAND;
		return;
	}
	...
	// After the second ground trace, so a step is only ever raised for a frame
	// that really ended with us standing on something.
	PM_Footsteps();
