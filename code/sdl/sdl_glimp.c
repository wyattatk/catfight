...
void GLimp_Init( qboolean fixedFunction )
{
	...
	{
		/*
		catfight -- "safe" is DESKTOP RESOLUTION, not 640x480.

		This block runs when the player answers yes to "the last run didn't
		exit properly, use safe video settings?". It used to set
		R_MODE_FALLBACK, and because r_mode is archived, that 640x480 was then
		saved and became the player's resolution permanently -- the dialog is
		offered once and the consequence is forever. Worse, it silently breaks
		fullscreen: exclusive fullscreen takes its mode from r_mode, so every
		later Alt+Enter mode-switched the whole monitor down to 640x480.

		That dialog fires whenever the process is killed rather than closed,
		which for this project is every automated test launch, so this was not
		a rare path.

		-2 is the display's own resolution. It cannot be wrong about the
		hardware the way a hardcoded size can, which makes it the safer "safe"
		on anything built this century. R_MODE_FALLBACK is still used below as
		the last-ditch retry after a mode has actually FAILED, which is the
		question it is a good answer to.
		*/
