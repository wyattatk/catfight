...
/*
Reading and writing key bindings.

These three have always existed in the engine's UI API (ui_public.h); catfight's
own module simply never declared them, because until there was something to
rebind nothing needed to ask. No engine change was required to add them.
*/
...
// catfight: what the companion remembers, for the stats screen.
int trap_VoiceMemory( char *buffer, int bufferSize ) {
	return syscall( UI_VOICE_MEMORY, buffer, bufferSize );
}

// catfight: the audio devices, for the audio screen. Returns the count; fills
// buffer with device `index` when one is asked for.
int trap_AudioDevices( int capture, int index, char *buffer, int bufferSize ) {
	return syscall( UI_AUDIO_DEVICES, capture, index, buffer, bufferSize );
}
