...
void R_AddIQMSurfaces( trRefEntity_t *ent ) {
	...
	if ( (ent->e.frame >= data->num_frames)
	     ...
	     || (ent->e.oldframe < 0) ) {
		/*
		catfight: SILENT WHEN THERE ARE NO FRAMES AT ALL.

		An IQM with num_frames 0 is a static model, which is a perfectly normal
		thing to ship -- catfight's placeholder weapon and its test cube are
		both one. But frame 0 is >= 0 frames, so the check above fires for every
		such model on EVERY FRAME, and asking for frame 0 of a model that has no
		frames is the only thing a caller can sensibly do. It is not a mistake
		and it should not be reported as one.

		Left as upstream in every other respect, including the clamp below,
		which is already a no-op in this case.

		Measured rather than assumed: one short session with the view model in
		hand wrote 11930 of these into a 12793-line qconsole.log. netplay's
		play-local.ps1 runs with developer 1 on purpose so the companion's
		situation feed reaches that log, and 93% spam makes it unreadable --
		this silences the noise rather than the signal.
		*/
