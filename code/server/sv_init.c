...
void SV_Init (void)
{
	...
	// catfight does not use a master server. Discovery goes through the
	// matchmaker (see netplay/README.md), which knows about servers because
	// they register with it, so there is nothing for a public server list to
	// do. Inheriting upstream's defaults here meant every public catfight
	// server announced itself to id's and ioquake3's masters -- other people's
	// infrastructure, advertising a game that is not theirs to list.
	//
	// Empty strings are skipped in SV_MasterHeartbeat, so this sends nothing
	// anywhere. Setting sv_master1 by hand still works if one is ever wanted.
#ifdef STANDALONE
