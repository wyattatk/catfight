...
static void SV_CheckTimeouts( void ) {
	...
	for (i=0,cl=svs.clients ; i < sv_maxclients->integer ; i++,cl++) {
		...
		/*
		Bots are exempt, because a timeout is meaningless for a client that
		never sends packets. Vanilla kept them alive by accident: botlib's AI
		calls SV_BotGetConsoleMessage every frame, which refreshes
		lastPacketTime as a side effect. catfight's companions have no botlib
		AI, so without this the engine drops every one of them with "timed out"
		exactly sv_timeout seconds after it spawns -- 90 seconds into a match,
		with nothing in the game module to explain where they went.
		*/
