...
//	Swap_Init();
	...
	/*
	catfight: "aasonly" brings up the navigation half and nothing else.

	The four setups below are the bot AI -- weapon weights, item goals, chat and
	movement -- and every one of them loads a CONFIG FILE out of botfiles/ that
	only ships with Quake 3. A game that has its own bots and only wants AAS for
	routing therefore cannot call BotLibSetup at all: BotSetupWeaponAI fails on
	a missing weapons.c, and because botlibsetup is set at the BOTTOM of this
	function the whole library stays unusable, so the next BotLibLoadMap reports
	"bot library used before being setup" -- which points at the caller's order
	of operations rather than at a missing file.

	Defaults to 0, so leaving it alone is exactly the behaviour this function
	has always had. Set it with BOTLIB_LIBVAR_SET before BOTLIB_SETUP.
	*/
	botlibglobals.aasonly = (int) LibVarValue("aasonly", "0");
...
#endif //DEMO
	//
	// catfight: mirror the setup. Tearing down subsystems that were never
	// brought up is how an opt-out turns into a crash on map change, and it
	// would land in botlib rather than in the flag that caused it.
	if (!botlibglobals.aasonly)
	{
