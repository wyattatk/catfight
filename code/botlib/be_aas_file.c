...
int AAS_LoadAASFile(char *filename)
{
	...
	{
		/*
		catfight: under "aasonly" this is a WARNING rather than a refusal.

		The check compares a number written by the AAS compiler against one
		computed by the engine (CM_Checksum, over eleven specific BSP lumps).
		That is only a staleness test while both sides compute it the same way,
		and NetRadiant-custom's mbspc -- the only AAS compiler still maintained,
		and the one in tools/ -- does not agree with ioquake3 here. Measured on
		an untouched map compiled minutes earlier: engine -209606026, aas file
		-1771053706. So for this toolchain the test does not detect staleness,
		it rejects every file unconditionally.

		Refusing on a number that is always wrong buys nothing, so it is
		downgraded rather than deleted -- deleting it would also hide the real
		stale case from anyone whose compiler DOES agree.

		WHAT REPLACES IT: staleness is prevented rather than detected.
		mapping/gen_aas.ps1 rebuilds the .aas whenever the .bsp is newer, and
		`cf_aas` reports what actually loaded. If she ever routes through a wall
		that exists, regenerate before reading a line of g_botroute.c.
		*/
		if (!botlibglobals.aasonly)
		{
		...
		botimport.Print(PRT_WARNING, "aas file %s checksum differs from the bsp "
			"(compiler disagreement, not necessarily stale) -- loading anyway\n",
			filename);
	} //end if
	//load the lumps:
	//bounding boxes
