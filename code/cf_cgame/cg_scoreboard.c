...
static void CG_RoundBanner( void ) {
	...
	case MS_ROUND_END:
		/*
		Read off who actually took the round, which the server now says outright
		in CS_SCORES.

		It used to be guessed from whether this player was still standing --
		"you either survived the round or you did not" -- and that is not the
		same question. In 2v2 they come apart every time somebody clutches: the
		player dies, his companion finishes it, and the banner told him ROUND
		LOST while the scoreboard behind it counted the round to his side. The
		scoreboard was right, which is the worst way for a HUD to disagree with
		itself.

		A draw is TEAM_FREE and is a real answer. The old test for it -- nobody
		has scored at all -- only worked for a draw in the first round of the
		match, and quietly stopped being true after that.
		*/
		if ( cgs.lastRoundWinner < 0 ) {
			// an older server that never sends it; "over" is all we honestly have
			text = "ROUND OVER";
			color = sbDim;
		} else if ( cgs.lastRoundWinner == TEAM_FREE ) {
			text = "DRAW";
			color = sbDim;
		} else if ( myTeam != TEAM_RED && myTeam != TEAM_BLUE ) {
			// a spectator has no side, so the round is neither won nor lost
			text = va( "%s WINS THE ROUND", CG_TeamName( (team_t)cgs.lastRoundWinner ) );
			color = CG_TeamColor( (team_t)cgs.lastRoundWinner );
		} else if ( cgs.lastRoundWinner == (int)myTeam ) {
			text = "ROUND WON";
	...
	/*
	Forgotten on the way out of the round end, and BEFORE the early return
	below. A live round with nothing to announce returns from here with no text
	at all, so a reset placed after that never runs -- and the second of two
	rounds that went the same way would be seen as the first one still on
	screen and never written down.
	*/
	if ( cgs.matchState != MS_ROUND_END ) {
		cg.lastBanner[0] = '\0';
	}
	...
	/*
	The round result, written down as well as drawn.

	A banner exists only as pixels, which is why this one was wrong for as long
	as it was: nothing outside the window could see it, so nothing could check
	it, and "ROUND LOST" on a round his side had just won survived every suite
	in netplay/. Logged with its two inputs beside it so the check is the whole
	rule on one line -- the winner and the reader's own side decide the word,
	and a line where they disagree with it is the bug.

	Once per round end rather than per frame: this is drawn sixty times a
	second for several seconds. Developer level, because the console belongs to
	the player. netplay/test-voicestate.ps1 reads these lines.
	*/
	if ( cgs.matchState == MS_ROUND_END && strcmp( text, cg.lastBanner ) ) {
		char dev[8];

		Q_strncpyz( cg.lastBanner, text, sizeof( cg.lastBanner ) );

		trap_Cvar_VariableStringBuffer( "developer", dev, sizeof( dev ) );
		if ( atoi( dev ) ) {
			CG_Printf( "banner: round end -- winner %i, my team %i -> %s\n",
			           cgs.lastRoundWinner, (int)myTeam, text );
		}
	}
