...
// catfight: what the companion is told about the match. Not console commands
// -- see the note in cgame/cg_public.h.
void trap_VoiceState( const char *situation ) {
	syscall( CG_VOICE_STATE, situation );
}

void trap_VoiceNote( const char *event ) {
	syscall( CG_VOICE_NOTE, event );
}

void trap_VoiceEvent( const char *kind, const char *detail ) {
	syscall( CG_VOICE_EVENT, kind, detail );
}

// The one that asks rather than tells. Returns a counter that moves whenever
// she says something new.
int trap_VoiceSubtitle( char *buffer, int bufferSize ) {
	return syscall( CG_VOICE_SUBTITLE, buffer, bufferSize );
}
