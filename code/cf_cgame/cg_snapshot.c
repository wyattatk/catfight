...
static void CG_SetInitialSnapshot( snapshot_t *snap ) {
	...
	/*
	And the event baseline with it. A map_restart comes through here and resets
	eventSequence, so a stale baseline would make the first frame of the new
	map look like a ring full of events nobody caused -- the same burst the
	first-frame guard in CG_DrawActiveFrame exists to prevent, arriving by a
	different route.
	*/
	cg.validOPS = qfalse;
...
static void CG_SetNextSnap( snapshot_t *snap ) {
	...
	for ( num = 0; num < snap->numEntities; num++ ) {
		...
		if ( !cent->currentValid ) {
			...
			/*
			A SLOT ARRIVING FRESH IS A DIFFERENT ENTITY, so forget what the last
			occupant announced.

			Missing this cost every impact effect in the game. Temp entities --
			bullet impacts, and the hit marker that rides on one -- are allocated
			from recycled entity slots, and CG_CheckEvents suppresses an event it
			has already fired by remembering it here. With nothing clearing that
			memory, each slot fired exactly ONE event for the whole map: markers
			and impact sounds worked for the first couple of seconds of the first
			firefight and were silently dropped from then on. It reads as
			"sometimes my hits register", and which shots happen to land in the
			working window is arbitrary.

			G_Spawn will not reuse a slot freed less than a second ago, which is
			twenty snapshots, so a slot cannot come back before this has run.
			*/
			cent->previousEvent = 0;
