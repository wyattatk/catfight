...
static void CG_ServerCommand( void ) {
	...
	/*
	Your companion answering an order.

	On screen rather than in the console, because the console is not up during a
	fight and an acknowledgment nobody sees does not do the job. The whole point
	of it is that the player can tell "she heard me" from "nothing happened" --
	which is the difference between a command system that feels alive and one
	that feels broken, and it matters far more once a language model is the thing
	producing the command and can be slow or wrong.

	It comes from the SERVER, never from whatever generated the command: only the
	server knows an order was actually accepted.
	*/
	if ( !strcmp( cmd, "cf_ack" ) ) {
		Q_strncpyz( cg.allyAck, CG_Argv( 1 ), sizeof( cg.allyAck ) );
		cg.allyAckTime = cg.time;
		return;
	}
