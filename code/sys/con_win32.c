...
// qtrue only when stdout is a real console. When output is redirected to a file
// or a pipe the console screen-buffer queries fail, and the input-line redraw in
// CON_Show must be skipped entirely -- see the comment in CON_Show.
static qboolean qconsole_isconsole = qfalse;
...
static void CON_Show( void )
{
	...
	// When stdout is not a console (redirected to a file or a pipe, which is how
	// automated testing runs the dedicated server) GetConsoleScreenBufferInfo
	// fails and leaves binfo uninitialised. The bounds arithmetic below then
	// indexes `line` with a garbage srWindow.Right and hands an out-of-bounds
	// pointer to WriteConsoleOutput, corrupting the heap a little on every
	// printed line until the process dies somewhere unrelated. There is no input
	// line to redraw in that case, so bail out. Actual log text is unaffected:
	// it goes out through CON_WindowsColorPrint's plain fputs.
	if( !qconsole_isconsole )
		return;
...
void CON_Init( void )
{
	...
	// this fails when stdout is redirected to a file or pipe rather than a
	// console; remember that so CON_Show can skip the input-line redraw
	qconsole_isconsole = GetConsoleScreenBufferInfo( qconsole_hout, &info ) ? qtrue : qfalse;
