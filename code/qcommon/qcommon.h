...
// catfight speaks its own protocol. Sharing ioquake3's 71 means a catfight
// client and an ioquake3 server recognise each other as compatible peers and
// then fail somewhere deep in the connection handshake, where the error tells
// nobody anything. A distinct number turns that into a clean, immediate
// version refusal. LEGACY_PROTOCOL is already off for standalone builds
// (q_shared.h), so there is no Quake 3 compatibility left to preserve here.
#ifdef STANDALONE
...
/*
catfight: a child process with pipes on its stdin and stdout.

Added for the Steam helper (code/client/cl_steam.c), which is a SEPARATE
PROGRAM on the far side of this interface and is the only way anything
Steam-related reaches the game. That separation is a licence requirement, not a
style preference -- see steam/README.md. Keeping the interface in terms of
bytes, with no notion of what the child is or does, is what makes it one.

Sys_ProcessRead NEVER BLOCKS. The engine pumps this from the main loop, and a
blocking read on a child that is waiting on a network round-trip to Valve would
freeze the frame.
*/
typedef struct sysProcess_s sysProcess_t;

// name is resolved next to the game binary, without an extension: the platform
// backend adds one where the platform has one. Returns NULL if it will not run.
...
// Returns bytes written, or -1 if the pipe is gone.
int Sys_ProcessWrite( sysProcess_t *proc, const char *data, int len );
// Returns bytes read, 0 if nothing is waiting, or -1 at EOF or on error.
