...
/*
The frame abort, and on Win64 it cannot be the one in <setjmp.h>.

Com_Error unwinds to the top of the frame loop, and the frame it unwinds FROM
is frequently inside a game module: cgame asks for a snapshot, the engine
notices the server dropped us, and ERR_DISCONNECT is raised with cgame.dll's
frames still on the stack.

On 64-bit Windows, msvcrt's longjmp is _setjmpex and performs a real SEH
unwind, which has to find valid unwind data for every frame it passes. Crossing
a game module's frames it does not, so RtlUnwindEx gives up and raises
0xC00000FF -- and the process dies with no message at all, because the message
was going to be printed by the code the longjmp never reached.

That is what "the game just quit instead of returning to the menu" was. It fires
on every match end, because that is when the server drops a client that is
mid-syscall. A disconnect noticed anywhere else -- a timeout, a server that went
away -- is detected at the top of the frame loop with no module frames in the
way, which is why only THIS one crashed and it looked like a match-end bug
rather than an error-handling bug.

GCC's builtins do a plain register-and-stack-pointer restore with no unwinding,
which is what this needs and what upstream ioquake3 settled on for the same
reason. The buffer is five words by the builtins' contract, and
__builtin_longjmp is only allowed to pass 1.
*/
#if defined(_WIN64) && defined(__GNUC__)
	#define Q_JMP_BUF		void *
	#define Q_JMP_BUF_SIZE	5
	#define Q_setjmp( b )	__builtin_setjmp( b )
	#define Q_longjmp( b )	__builtin_longjmp( b, 1 )
#else
	#define Q_JMP_BUF		jmp_buf
	#define Q_JMP_BUF_SIZE	1
	#define Q_setjmp( b )	setjmp( b )
	#define Q_longjmp( b )	longjmp( b, -1 )
#endif

Q_JMP_BUF abortframe[Q_JMP_BUF_SIZE];	// an ERR_DROP occurred, exit the entire frame
