...
catfight: the stable per-install identifier the matchmaker uses.

This exists because cl_guid CANNOT do the job. cl_guidServerUniq defaults to 1
(see below), so cl_guid is hashed with the server's address mixed in -- a
per-install per-SERVER value, different on every port in a server pool, and
deliberately so, to stop operators tracking players between servers. It is also
only computed inside CL_Connect_f, so before the first connection it is empty,
and cfmm is talked to well before any game server is involved.

Same file, no prefix, computed once at startup and never touched again.
====================
*/
void CL_UpdatePlayerKey( void )
{
	fileHandle_t f;
	int len;
	...
	if( len != QKEY_SIZE )
		Cvar_Set( "cl_playerKey", "" );
	else
		Cvar_Set( "cl_playerKey", Com_MD5File( QKEY_FILE, QKEY_SIZE, NULL, 0 ) );
}
...
catfight -- go fullscreen AT A RESOLUTION THAT MAKES SENSE.

`r_fullscreen 1` on its own is not enough and that is the whole reason this
exists. ioquake3's fullscreen is exclusive, so it takes the mode from r_mode
and hands it to the display: with a stale r_mode the monitor is genuinely
switched down to that resolution. Toggling fullscreen with r_mode 3 does not
fail, it succeeds -- at 640x480, on a 1440p monitor, mode-switching the whole
desktop. Which reads, correctly, as "I cannot fullscreen the game".

r_mode 3 is not an exotic setting to be carrying. It is what the engine itself
writes when the player answers yes to the "last run didn't exit properly, use
safe video settings?" dialog, and that dialog fires every time the process is
killed rather than closed -- which, for this project, is every automated test
launch. One yes pins 640x480 windowed into the saved config permanently,
because the archived config beats default.cfg.

So going fullscreen forces desktop resolution. r_mode -2 asks the display what
it is rather than asserting a size, which is the only value that is right on a
machine nobody has measured. Leaving fullscreen does NOT put r_mode back: the
player is returning to a window, and a window at desktop resolution is fine.

A vid_restart is needed only when r_mode actually changed. Otherwise
GLimp_EndFrame does the cheap SDL toggle on the existing window, which is
instant and keeps the map loaded.
=================
*/
void CL_SetFullscreen( qboolean fullscreen ) {
	qboolean modeChanged = qfalse;

	if ( fullscreen && Cvar_VariableIntegerValue( "r_mode" ) != -2 ) {
		Com_Printf( "fullscreen: using desktop resolution (r_mode -2)\n" );
		Cvar_Set( "r_mode", "-2" );
		modeChanged = qtrue;
	}

	Cvar_SetValue( "r_fullscreen", fullscreen ? 1 : 0 );
...
`cf_fullscreen` with no argument toggles; with one it sets.

Both forms exist because the two callers want different things: a key toggles,
and a menu checkbox sets the value it is showing. A menu that toggled would
drift out of step with its own tickbox the first time anything else changed
the cvar.
=================
*/
static void CL_Fullscreen_f( void ) {
	qboolean want;

	if ( Cmd_Argc() > 1 ) {
		want = (qboolean)( atoi( Cmd_Argv( 1 ) ) != 0 );
	} else {
		want = (qboolean)( !Cvar_VariableIntegerValue( "r_fullscreen" ) );
	}

	CL_SetFullscreen( want );
}
...
void CL_Frame ( int msec ) {
	...
	// catfight: drain the Steam helper's pipe before matchmaking looks at it,
	// so a ticket that arrived this frame is usable this frame.
	CL_Steam_Frame();

	// catfight: and the companion helper's. Before CL_SendCmd, so a cf_cmd
	// that arrived this frame reaches the server this frame -- a command that
	// waits a frame for no reason is a command she answered slower than she
	// had to, and the whole feature is judged on whether she feels prompt.
	CL_Voice_Frame();

	// catfight: advance matchmaking. After cls.realtime is updated, because the
	// poll timer keys off it; before CL_SendCmd, so a match found this frame
	// starts connecting this frame rather than next.
	CL_MM_Frame();
...
void CL_Init( void ) {
	...
	Cmd_AddCommand ("cf_fullscreen", CL_Fullscreen_f);
...
//	Cbuf_Execute ();
	...
	// catfight: stable identity for the matchmaker, and the mm_* commands.
	Cvar_Get( "cl_playerKey", "", CVAR_ROM );
	CL_UpdatePlayerKey();
	// Before CL_MM_Init: starting the helper here gives its handshake the whole
	// of startup to complete, so the first mm_find does not wait on it.
	CL_Steam_Init();
	CL_MM_Init();
	// Last, and for the same reason CL_Steam_Init is early: loading the weights
	// takes seconds, so it is started at boot and has finished long before
	// anyone speaks to her. Nothing else waits on it, so it goes after the
	// things that do.
	CL_Voice_Init();
...
void CL_Shutdown(char *finalmsg, qboolean disconnect, qboolean quit)
{
	...
	// catfight: before CL_HTTP_Shutdown, which joins the worker threads that
	// any in-flight matchmaking request is sitting on.
	CL_MM_Shutdown();
	CL_Steam_Shutdown();
	// Sends quit and closes the pipe, which is what makes cfvoice exit and take
	// several gigabytes of resident model down with it. A leaked helper after
	// the game closes is the kind of thing players notice and blame on us.
	CL_Voice_Shutdown();
