...
Whether a +button binding should still fire while a menu is up.

catfight: almost none should, and that is the rule this is an exception to.
Holding forward on the home screen must not walk the player around behind it,
which is why allCommands below requires nothing at all to be catching keys.

The companion is the exception, deliberately. Talking to her is the ONE thing
there is to do between matches, and the home screen is where the time between
matches is spent -- so a push-to-talk key that works only while a round is live
is a companion you cannot say anything to about the round that just ended. She
is running the whole time the game is; only the key was gated.

Not while the console or the chat field has the keys: there the key is a letter
somebody is typing, and 'b' is a common one.

If the console is opened between the press and the release, the release is lost
and recording would stick -- except that cl_voice.c caps a clip at
VOICE_MAX_SECONDS and finishes it itself, which is the same backstop that
already covers a key held through an alt-tab.
===================
*/
static qboolean CL_BindUIButton( const char *cmd ) {
	if ( Key_GetCatcher( ) & ( KEYCATCH_CONSOLE | KEYCATCH_MESSAGE ) )
		return qfalse;

	return (qboolean)( !Q_stricmp( cmd, "voicerecord" ) );
}
...
void CL_ParseBinding( int key, qboolean down, unsigned time )
{
	...
	while( 1 )
	{
		...
		if( *p == '+' )
		{
			...
			// subframe corrected
			// CL_BindUIButton is checked before the up-command rule rather
			// than folded into it, because it must let the press through too
			// -- and because at the home screen allowUpCmds is false, so the
			// release would otherwise be dropped and the clip never finish.
			if ( allCommands || CL_BindUIButton( p + 1 )
				 || ( allowUpCmds && !down ) ) {
				char cmd[1024];
...
void CL_KeyDownEvent( int key, unsigned time )
{
	...
	if( keys[K_ALT].down && key == K_ENTER )
	{
		...
		/*
		catfight -- through CL_SetFullscreen rather than setting the cvar
		directly, so that this fixes r_mode on the way.

		Setting r_fullscreen alone is what this used to do, and it is why
		Alt+Enter appeared not to work: it worked, and went fullscreen at
		whatever stale resolution r_mode was carrying. Alt+Enter is the only
		fullscreen control most players will ever look for, so it is the one
		place this must not be subtly wrong.
		*/
		CL_SetFullscreen( (qboolean)( !Cvar_VariableIntegerValue( "r_fullscreen" ) ) );
		return;
	}
