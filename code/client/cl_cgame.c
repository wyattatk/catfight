...
intptr_t CL_CgameSystemCalls( intptr_t *args ) {
	...
	// catfight: the companion's feed. Straight through rather than via the
	// console -- see the note in cgame/cg_public.h.
	case CG_VOICE_STATE:
		CL_Voice_State( VMA(1) );
		return 0;
	case CG_VOICE_NOTE:
		CL_Voice_Note( VMA(1) );
		return 0;
	case CG_VOICE_EVENT:
		CL_Voice_Event( VMA(1), VMA(2) );
		return 0;
	// The only one of these that answers. Same shape as CG_ARGS above: a
	// buffer in, a length, a return value out.
	case CG_VOICE_SUBTITLE:
		return CL_Voice_Subtitle( VMA(1), args[2] );
	case CG_ADDCOMMAND:
