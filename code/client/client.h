...
//
// cl_steam.c -- catfight's side of the Steam helper process
//
// Nothing here names a Steamworks type on purpose. The helper is a separate
// program precisely so that no proprietary SDK is linked into this GPL binary;
// see the header comment in cl_steam.c and steam/README.md.
//
typedef enum {
	STEAMTICKET_NONE,
	STEAMTICKET_PENDING,
...
// Generous on purpose. A web-api ticket is on the order of a kilobyte of
// binary, so two kilobytes of hex, and the failure mode of a buffer that is
// merely snug is the worst kind: the ticket is silently truncated, Valve
// rejects it, and the error blames the credential -- which sends you looking
// at the publisher key rather than at this number. The same mistake in the
// match-ticket buffer cost real time in Phase 4c.
#define MAX_STEAM_TICKET	8192
...
//
// cl_voice.c -- catfight's side of the companion helper process
//
// Nothing here names a model, a prompt or a persona, for the same reason
// nothing above names a Steamworks type: the helper is a separate program, and
// the companion's mind lives on the far side of a pipe. See the header comment
// in cl_voice.c and cfvoice/README.md.
//
void CL_Voice_Init( void );
void CL_Voice_Frame( void );
void CL_Voice_Shutdown( void );
qboolean CL_Voice_Available( void );
// Fed by cgame, which is the only side that knows what a stat means.
void CL_Voice_State( const char *situation );

// catfight -- fullscreen that also fixes the resolution it goes fullscreen at.
// See CL_SetFullscreen; a bare `r_fullscreen 1` is not enough.
...
// The last line she spoke, for cgame to subtitle. Returns a counter that moves
// when the line changes; see CL_Voice_Subtitle on why it is not a timestamp.
int CL_Voice_Subtitle( char *buf, int size );
void CL_Voice_Note( const char *event );

//
// cl_mm.c -- catfight matchmaking client
//
