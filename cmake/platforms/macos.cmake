...
# Universal by default, but let -DCMAKE_OSX_ARCHITECTURES override it.
#
# This was a plain set(), which creates a normal variable that shadows the cache
# entry for the rest of the configure -- so -DCMAKE_OSX_ARCHITECTURES was
# silently ignored and every macOS build came out universal no matter what was
# asked for. netplay/macos/build-server.sh had been passing arm64 for weeks with
# no effect whatsoever; the server ran on the Intel host only because the
# x86_64 slice it did not want was there anyway.
if(NOT CMAKE_OSX_ARCHITECTURES)
    set(CMAKE_OSX_ARCHITECTURES arm64;x86_64)
endif()

# The .app bundle is a client concept -- finish_macos_app() reaches for
# ${CLIENT_BINARY}, which is only ever a target when BUILD_CLIENT is on. A
# dedicated-server-only macOS build (BUILD_CLIENT=OFF) otherwise dies at
# configure time inside set_target_properties on an empty target name, and the
# error says nothing about app bundles. Server hosts are exactly the machines
# that build this way.
if(BUILD_MACOS_APP AND BUILD_CLIENT)
