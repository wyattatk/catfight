...
* The matchmaker, the server agent, the Steam helper, and the companion helper.
  The first two are server software that is never distributed. The other two are
  separate programs that communicate with the game only over a text protocol on
  a pipe; they share no source with the engine and are not part of this work.
  The Steam helper links the proprietary Steamworks SDK. The companion helper
  (cfvoice) runs a language model and holds the character's persona and memory.
